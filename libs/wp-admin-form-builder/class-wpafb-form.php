	...
	class WPAFB_Form
	{
		...
		private function _render($short_data, $call_option = array("group" => false))
		{
			...
			if ($group) {
				...
				if ($this->single_column) {
					$data['single_column'] = true;
				} //force to render this field in signle column
				...
				if ($group_type == "row") {
					...
					if ($fieldset) {
						...
						echo '<legend>' . $legend . $help_icon . '</legend>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					...
					echo $this->get_field_description($data); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '</td>';
					...
					if ($single_column === false) {
						echo '<td class="wpafb-left-col"><strong><label>' . $label . '</label></strong></td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}

					echo '<td class="' . $rcol_class . '" colspan="' . $colspan . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					if ($single_column === true && $label != '') {
						echo '<p><strong><label>' . $label . '</label></strong></p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
					...
					foreach ($fields as $key => $short_args) {
						...
						if ($type == "group") {
							...
							echo '<label class="' . $fl_class . '" for=' . esc_attr($fid) . '>' . $fl . '</label><br>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					foreach ($fields as $key => $short_args) {
						...
						if ($type == "group") {
							continue;
						} //do not allow group in a adjacent-horizontal group
						$args['used_in_group_type'] = $group_type;
						$fid = $args['id'];
						$fl =  isset($args['label']) ? $args['label'] : "";
						$fl_class = ($this->is_disabled_field($args)) ? "disabled" : "";
						if ($fl != "") {
							if ($i == 0) {
								echo '<label class="' . $fl_class . '" for=' . esc_attr($fid) . '>' . $fl . ' '; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							} else {
								echo ' <label class="' . $fl_class . '" for=' . esc_attr($fid) . '>' . $fl . ' '; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							} //For first item use only after space.
						}
						$this->call_callback($args);
						if ($fl != "") {
							echo '</label> ';
					...
					if (! $fieldset) {
						echo $help_icon; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
			...
			} else // else of if( $group )
			{
				...
				foreach ($fields as $key => $short_args) {
					...
					if ($type == "group") {
						...
						if ($single_column === false) {
							echo '<td class="wpafb-left-col"><strong><label class="' . $fl_class . '" for=' . esc_attr($id) . '>' . $label . '</label></strong></td>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}

						echo '<td class="' . $rcol_class . '" colspan="' . $colspan . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						if ($single_column === true && $label != '') {
							echo '<p><strong><label class="' . $fl_class . '" for=' . esc_attr($id) . '>' . $label . '</label></strong></p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
						$this->call_callback($args);
						echo '</td>';
						echo '</tr>';
		...
		public function is_disabled_field($args)
		{
			...
			if ($disabled_opt === true) {
				return true;
			} //can be an array of item keys for radio and multicheck. We will not disable that if all options are not disabled by provideing 'true'.
			return false;
		}
		...
		public function form_header()
		{
			...
			echo $this->header; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public function form_footer()
		{
			...
			echo $this->footer; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public function callback_header($args)
		{
			...
			echo '<' . $tag . ' id="' . esc_attr($id) . '" class="' . esc_attr($class) . '" ' . $fieldtype . $attrs_string . '>' . $value . '</' . $tag . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		public function callback_multicheck($args)
		{
			...
			if (!is_array($disabled_opt) && $disabled_opt !== true && $disabled_opt !== false) {
				$disabled_opt = false;
			}
			if (empty($disabled_opt)) {
				$disabled_opt = false;
			}
			if ($disabled_opt === true) {
		...
		public function callback_html($args)
		{
			...
			echo isset($args['value']) ? $args['value'] : ""; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		public function callback_editor($args)
		{
			...
			echo '<div id="' . esc_attr($id . "_wrapper") . '" class="' . esc_attr($class) . '"' . $formid . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			...
			echo $this->get_help_icon($args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo $this->get_after_html($args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '</div>';
		...
		public function callback_image($args)
		{
			...
			$this->callback_file($args, true); // ture for image type  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		public function callback_radio_image($args)
		{
			...
			foreach ($options as $key => $option) {
				...
				if ($disabled) {
					$item_box_class .= ' disabled';
				}
				if (trim($checked) != '') {
					$item_box_class .= ' selected';
				}
		...
		private function add_fields($fields)
		{
			...
			foreach ($fields as $key => $args) {
				...
				if ($id != $key) {
					continue;
				} //array key and the id value must be same.
				if ($type == 'group') {
					if (!isset($args['group_type']) || !in_array($args['group_type'], array('row', 'horizontal', 'vertical'))) {
						continue;
					}
				}
		...
		private function get_value_from_request_when_html_name_array($html_name, $id)
		{
			...
			foreach ($keys as $key) {
				if ($key == "") {
					return null;
				} // 'hello[]', 'hello[][test][]', 'hello[][test][]' types of name are not allowed
				if (isset($value[$key])) {
					$value = $value[$key];
				} else {
					return null;
