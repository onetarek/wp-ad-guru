...
$triggering_form_args = array(
	'id' => 'winp_triggering_form',
	'header_callback' => 'adguru_window_popup_form_triggering_header_callback',
	'footer_callback' => 'adguru_window_popup_form_triggering_footer_callback',
	'fields' => array(
		/*
		...
		'triggering_auto_open_delay' => array(
			...
			'step' => 10,
			'display_text' => 'ms',
			'help' => 'Number in miliseconds'
		),*/
		'triggering_open_on_body_click_enable' => array(
			'type' 	=> 'checkbox',
			'id'	=> 'triggering_open_on_body_click_enable',
			'label'	=> __("Open on BODY click", 'wp-ad-guru' ),
			'default'	=> '1',
			'readonly' => true,
			'help' => __("Popup open when user clicks anywhere of the page", 'wp-ad-guru' ),
		),
...
function adguru_window_popup_form_triggering_header_callback( $form_obj )
{
	do_action('adguru_editor_form_window_popup_triggering_top', $form_obj );
}

function adguru_window_popup_form_triggering_footer_callback( $form_obj )
{
	?>
	...
	<?php
	do_action('adguru_editor_form_window_popup_triggering_bottom', $form_obj );
}

//Apply filters so that extensions can add/modify fields settings
$triggering_form_args = apply_filters('adguru_editor_form_window_popup_triggering_args', $triggering_form_args );
//reassign header and footer callback to prevent modification the value of  header and  footer callback key.
$triggering_form_args['header_callback'] = 'adguru_window_popup_form_triggering_header_callback';
$triggering_form_args['footer_callback'] = 'adguru_window_popup_form_triggering_footer_callback';
//Create the form object
$triggering_form = adguru()->form_builder->create_form($triggering_form_args);


function adguru_show_window_popup_triggering_form( $ad )
{
	...
	{ 
		...
		do_action('adguru_editor_form_window_popup_triggering_before_render', $triggering_form );
		//render the form
